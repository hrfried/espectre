...
void WiFiLifecycleManager::ip_event_handler_(void* arg, esp_event_base_t event_base,
  ...
  if (event_id == IP_EVENT_STA_GOT_IP) {
    ...
    ESP_LOGI(TAG, "📡 WiFi Promiscuous mode: %s", promiscuous ? "ENABLED" : "DISABLED");
    
    wifi_ps_type_t ps_type;
    esp_wifi_get_ps(&ps_type);
    const char* ps_str = (ps_type == WIFI_PS_NONE) ? "NONE" : 
                         (ps_type == WIFI_PS_MIN_MODEM) ? "MIN_MODEM" : "MAX_MODEM";
    ESP_LOGI(TAG, "📡 WiFi Power Save: %s", ps_str);
    
    uint8_t protocol = 0;
    esp_wifi_get_protocol(WIFI_IF_STA, &protocol);
    ESP_LOGI(TAG, "📡 WiFi Protocol: 0x%02X (802.11b=%d, 802.11g=%d, 802.11n=%d, 802.11ax=%d)", 
             protocol,
    ...
    ESP_LOGI(TAG, "📡 WiFi Bandwidth: %s", (bw == WIFI_BW_HT20) ? "HT20" : "HT40");
